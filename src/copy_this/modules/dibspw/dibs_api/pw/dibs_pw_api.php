...
class dibs_pw_api extends dibs_pw_helpers {
    ...
    final public function api_dibs_get_requestFields($mOrderInfo) {
        ...
        $this->api_dibs_prepareDB($mOrderInfo->getOrderId());
    ...
    private function api_dibs_commonFields(&$aData, $oOrder) {
        ...
        $aData['merchant'] = $this->helper_dibs_tools_conf('merchantId');
        $aData['amount']   = self::api_dibs_round($oOrder->order->amount);
        $aData['currency'] = $oOrder->order->currency;
        $aData['language'] = $this->helper_dibs_tools_conf('langpw');
        if((string)$this->helper_dibs_tools_conf('addFee') == 'yes') $aData['addFee'] = 1;
    ...
    private function api_dibs_invoiceFields(&$aData, $mOrderInfo) {
        ...
        $sDistribType = $this->helper_dibs_tools_conf('distrType');
    ...
    final public function api_dibs_checkMainFields($mOrderInfo, $bUrlDecode = TRUE) {
        ...
        if(!$mOrderInfo->oxorder__oxid->getRawValue()) return 2;
        $iAmount = (isset($_POST['voucherAmount']) && $_POST['voucherAmount'] > 0) ? 
                    $_POST['amountOriginal'] : $_POST['amount'];
  
        //if(abs((int)$iAmount - (int)self::api_dibs_round((float)$mOrderInfo->oxorder__oxtotalordersum->getRawValue())) >= 0.01) return 4;
        if((int)$mOrderInfo->oxorder__oxcurrency->getRawValue() != (int)$_POST['currency']) return 6;
    ...
    private function api_dibs_updateResultRow($aFields) {
        if(isset($_POST['orderid']) && !empty($_POST['orderid'])) {
            $ordId = $this->helper_getOrderId($_POST['orderid']);    
            $sUpdate = '';
            ...
            $this->helper_dibs_db_write(
                "UPDATE `" . $this->helper_dibs_tools_prefix() . self::api_dibs_get_tableName() . "`
                 SET " . rtrim($sUpdate, ",") . " 
                 WHERE `orderid` = '" . self::api_dibs_sqlEncode($ordId) . "' 
                 LIMIT 1;"
            );
        }
    }
